...
sentence = "Where do you think you're going? What's up, Doc?".strip()
...
def sentences(text):
    ...
    # Step through pairs: text + punctuation    
    for i in range(0, len(parts), 2):
        sentence_part = parts[i].strip()
        if sentence_part:
            punctuation = parts[i+1] if i + 1 < len(parts) else '' #check if punctuation exists
            sentences.append(sentence_part + punctuation)
...
def longest_sentence(text):
    ...
    if not all_sentences:
        print("No sentences found in the input.")
        return

    max_words = 0
    max_sentence = ""

    for content in all_sentences:
        words = content.split()
        if len(words) > max_words:
            max_words = len(words)
            max_sentence = content

    print(f"{max_sentence}\n")
    print(f"The longest sentence has {max_words} words.\n")
